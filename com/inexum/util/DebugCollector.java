...
{
    static class NoDebug extends Throwable {}
    ...
    public void run()
    {
        ...
        ProcessingQueue queue = Instance().queue();
        while (!m_halt && queue != null)
        {
            try 
            {
                out((String)queue.waitElement());
            }
            catch (Exception e)
            {
                e.printStackTrace();
    ...
    protected DebugCollector() throws NoDebug
    {
        String debug = System.getProperty("microac.debug");
        if (debug == null) throw new NoDebug();
